...
The current Windows release is `3.0.0`. It uses the compiled WPF host, monitors Codex Desktop and Codex CLI sources, keeps the on-demand Settings compatibility process, and retains `scripts/start.ps1 -Legacy` as a recovery path.
...
- The shared state table spans enabled Desktop, normal CLI, and isolated DeepSeek CLI profiles under one global discovery and task-number bound.
- Discovery considers recent writes across all session creation-date folders and is capped at 64 files. On Windows, read-only `state_5.sqlite` activity is reconciled every two seconds to recover top-level `user` tasks when Codex resumes an old parent JSONL. Three minutes controls only the fresh active classification; discovery retains that recovered task for `activeWindowMinutes` (30 minutes by default), so a long silent inference stays listed without remaining falsely green forever. Full directory discovery remains bounded and infrequent.
...
- Source identity is explicit before every list/split task number: Desktop window, OpenAI CLI terminal, or DeepSeek CLI wave-terminal. Unknown future model IDs remain monitorable.
...
- Summary, list, and split views share accounting and lifecycle semantics. The summary contains only meaningful cross-task totals/account observations; model, context, cache hit rate, task cumulative total, and timestamps remain per-task.
- Visible turn completion and aborted states require explicit lifecycle records. Silent turn boundaries are ignored, visible completion is guarded against immediate continuation, and timeouts become idle.
...
- Runtime liveness is an injected Core interface. The Windows App implements it through the system `winsqlite3.dll` in read-only mode; the retained legacy host has the same fallback. Only identity/path/timestamp/source-class/archive fields are selected, and any missing database, lock, or schema drift falls back to bounded JSONL discovery.
...
- The MCP server negotiates current revision `2025-11-25` plus compatible older revisions and exposes source-aware structured task results. Plugin installation remains optional and per `CODEX_HOME`.
- Closing an independent task bubble now merges only that bubble back into the main HUD. It no longer invokes task dismissal or removes the task from aggregate monitoring.
- List density is progressive rather than field-order driven: Compact keeps model/context/cache efficiency, Balanced adds the current call total, and Detailed wraps diagnostic accounting plus the provider-reported context-window capacity onto a second line. Context usage is always visible in the list and uses the task's own GPT or DeepSeek window.
- Preset and custom placement account for the transparent 18 px HUD chrome (16 px for task bubbles), allowing the visible shell to touch the work-area edge without clipping its functional content.
...
- API-equivalent cost is an estimate based on local pricing data, not a bill or exact credit conversion. The built-in `3.0.0` snapshot is standard short-context API list price only; it excludes long-context, Batch, Flex, Fast mode, regional-processing, and cache-write prices.
- `3.0.0` is Windows x64 only. macOS is not supported or packaged; community source adaptations are welcome but are outside this project's validation and release scope.
- CLI monitoring currently recognizes the normal profile and the conventional isolated `~/.codex-deepseek` profile. Other arbitrary profile roots are not user-configurable in this candidate.
- The supplemental state-database heartbeat depends on Codex's current `threads` schema. It is deliberately optional and fail-closed: if those non-content columns change, monitoring continues through JSONL without editing or migrating the database.
