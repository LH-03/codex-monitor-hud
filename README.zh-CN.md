...
> [English guide / English documentation](README.md) · **简体中文说明**

一个面向 Windows 版 Codex Desktop 的轻量、精致、完全本地运行的实时监控气泡。

它不是追求功能大而全的“事后用量统计面板”，也不会建立另一套历史数据库。它的核心场景是：让 Codex 在后台自己运行任务，你在前台看视频、打游戏或处理别的工作，只留下一个很小的悬浮气泡，就能实时知道任务仍在活动、正在监听、已经空闲、被暂停或发生读取错误，并看到最新 Token 和本地观测到的额度状态。

因此这个项目优先保证三件事：足够轻量、刷新灵敏、长期监控稳定可靠。丰富的历史报表不是它要竞争的方向。
...
- 高级状态颜色与时间阈值。
- 可选鼠标穿透，默认关闭；穿透时 HUD 继续显示和刷新，鼠标操作落到下层应用，并可从托盘恢复；
- 4 套状态颜色方案，并保留每种状态单独输入 ARGB；字号以 0.1 为步进连续调整。
...
- 双击：打开设置；
- 右键：暂停、恢复、重置位置或退出；
- 可从设置或右键菜单开启鼠标穿透。开启后请在 Windows 托盘找到常驻的 Codex Token HUD 图标，右键选择“关闭鼠标穿透 / Disable click-through”；托盘也能打开设置。桌面/开始菜单设置快捷方式和让 Codex“关闭 Token HUD 鼠标穿透”仍是备用入口；
