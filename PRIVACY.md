...
The workspace leaf and Codex-provided thread name are used only for per-task identity in list and split views. Lifecycle reason text and assistant-message content are not used to infer semantic completion. The full working-directory path and prompt/conversation text are never displayed or stored by the HUD. Task numbers and conversation labels exist only in runtime memory; the privacy-safe MCP registry stores the task number, workspace leaf, coarse status, and update time, but not the conversation label.
